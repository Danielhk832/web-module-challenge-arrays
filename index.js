...
function addFlavor(array, addition){
...
function removeFlavorByName(array, removal){
...
function getRandomFlavors(array1, array2, array3, array4){
  const concatArray = originalFlavors.concat(regionalFlavors, seasonalFlavors, newFlavors);
  const newArray = [];
  let randomSelection;

  for(let i = 0; i < 31; i++){
    randomSelection = Math.floor(Math.random() * 102 + 1);
    newArray.push(concatArray[randomSelection]);
  }
  return newArray;
}
